...
def aave_supply_1():
    ...
    w3.eth.send_transaction(unsigned_transaction).hex()
    time.sleep(2)
    print(f"⛽️ GAS ESTIMATE: {gas_estimate} ⛽️ -- aave supply: USDC")
...
def aave_borrow_1():
    ...
    w3.eth.send_transaction(unsigned_transaction).hex()
    time.sleep(2)
    print(f"⛽️ GAS ESTIMATE: {gas_estimate} ⛽️ -- aave borrow: WETH")
...
def uniswap_1():
    ...
    w3.eth.send_transaction(unsigned_transaction).hex()
    time.sleep(2)
    print(f"⛽️ GAS ESTIMATE: {gas_estimate} ⛽️ -- uniswap sell: WETH for USDC")
...
def aave_borrow_2():
    res = compass.aave_v3.borrow(
        token=models.TokenEnum.WETH,
        amount=Decimal(0.002015764334050235381693634693),
        interest_rate_mode=models.InterestRateMode.VARIABLE,
        chain=chain,
        sender=WALLET,
        server_url="http://0.0.0.0:80",
