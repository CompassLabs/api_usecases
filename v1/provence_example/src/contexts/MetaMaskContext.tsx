...
export function MetaMaskProvider({ children }: { children: ReactNode }) {
  ...
  // Check for existing connection on mount and handle reconnection
  useEffect(() => {
    const checkConnection = async () => {
      ...
      try {
        ...
        // Clear any previous error when checking connection
        setError(null);
        ...
        if (accounts?.length > 0) {
          ...
          // No accounts found, but MetaMask is available
          console.log('MetaMask available but no accounts connected');
          setWallet(null);
        }
      } catch (err: any) {
        console.error('Error checking existing connection:', err);
        // Don't set error for initial connection check unless it's a critical error
        if (err.message?.includes('No active wallet')) {
          console.log('MetaMask available but locked or no active wallet');
          setWallet(null);
        }
      }
    };
    ...
    // Also check connection when the window gains focus (user returns from MetaMask)
    const handleFocus = () => {
